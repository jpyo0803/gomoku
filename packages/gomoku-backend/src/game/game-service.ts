...
export class GameService {
  ...
  handleMatchRequest(playerId: string, wantAiOpponent: boolean) {
    ...
    console.log(`Match request from player ${playerId}, want AI: ${wantAiOpponent}`);
    if (wantAiOpponent) {
        // AI 상대 매칭, Queue 필요없음
      ...
      this.socketGateway.sendYourTurn(playerId, 30); // 유저에게 턴 알림
      return gameId;
    }
    // 일반 플레이어 매칭
    // 아직 구현 안됨
  ...
  async handlePlaceStone(playerId: string, x: number, y: number) {
    ...
    console.log(`Player ${playerId} placed stone at (${x}, ${y}), result: ${result}`);

    // 응답
    this.socketGateway.sendPlaceStoneResp(playerId, result);
